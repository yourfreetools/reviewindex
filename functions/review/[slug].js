...
export async function onRequest(context) {
    ...
    try {
        ...
        // Generate cache key for the full page
        const pageCacheKey = `page-${slug}`;
        const cacheUrl = new URL(request.url);
        
        // Check Cloudflare cache for the full page first
        ...
        // Get related posts WITH CLOUDFLARE CACHE
        const relatedPosts = await findRelatedPostsWithCloudflareCache(
            currentFrontmatter, 
            slug, 
            env.GITHUB_TOKEN
        );
        ...
        const response = new Response(fullHtml, {
            headers: { 
                ...
                'Cloudflare-CDN-Cache-Control': 'public, max-age=15552000'
            }
        });

        // Store in Cloudflare cache
        context.waitUntil(cache.put(cacheUrl, response.clone()));
        console.log('💾 Page cached for 6 months:', slug);
...
// ==================== CLOUDFLARE CACHE FOR RELATED POSTS ====================

async function findRelatedPostsWithCloudflareCache(currentFrontmatter, currentSlug, githubToken) {
    const cacheKey = `related-${currentSlug}`;
    
    try {
        // 1. CHECK CLOUDFLARE CACHE FIRST using custom cache API pattern
        const cache = caches.default;
        const cacheRequest = new Request(`https://cache.reviewindex.pages.dev/${cacheKey}`);
        let cachedResponse = await cache.match(cacheRequest);
        
        if (cachedResponse) {
            console.log('✅ Related Posts Cache HIT for:', currentSlug);
            const cachedData = await cachedResponse.json();
            return cachedData;
        }
        
        console.log('🔄 Related Posts Cache MISS for:', currentSlug, '- Fetching from GitHub...');
        ...
        // 3. STORE IN CLOUDFLARE CACHE (6 MONTHS)
        if (relatedPosts.length > 0) {
            const cacheResponse = new Response(JSON.stringify(relatedPosts), {
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'public, max-age=15552000', // 6 months
                    'CDN-Cache-Control': 'public, max-age=15552000'
                }
            });
            
            context.waitUntil(cache.put(cacheRequest, cacheResponse));
            console.log('💾 Related posts cached for 6 months:', currentSlug, '-', relatedPosts.length, 'posts');
        }
...
async function findRelatedPostsFromGitHub(currentFrontmatter, currentSlug, githubToken) {
    try {
        // Get all posts metadata WITH CLOUDFLARE CACHE
        const allPosts = await fetchAllPostsMetadataWithCache(githubToken);
...
async function fetchAllPostsMetadataWithCache(githubToken) {
    const cacheKey = 'all-posts-metadata';
    ...
    try {
        ...
        if (cachedResponse) {
            console.log('✅ Posts Metadata Cache HIT');
            const cachedData = await cachedResponse.json();
            return cachedData;
        }
        
        console.log('🔄 Posts Metadata Cache MISS - Fetching from GitHub...');
        
        // Fetch fresh data from GitHub
        const postsMetadata = await fetchAllPostsMetadata(githubToken);
        
        // Store in Cloudflare cache for 6 months
        if (postsMetadata.length > 0) {
            const cacheResponse = new Response(JSON.stringify(postsMetadata), {
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'public, max-age=15552000', // 6 months
                    'CDN-Cache-Control': 'public, max-age=15552000'
            ...
            context.waitUntil(cache.put(cacheRequest, cacheResponse));
            console.log('💾 Posts metadata cached for 6 months:', postsMetadata.length, 'posts');
        }
        
        return postsMetadata;
        
    } catch (error) {
        console.error('Error in fetchAllPostsMetadataWithCache:', error);
        return [];
    }
}
