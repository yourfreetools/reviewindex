...
<head>
    ...
    <style>
        ...
        * { 
        ...
        /* FIX FOR EMPTY SPECIFICATIONS SPACING */
        .product-info-section:empty {
            display: none;
        }
        
        /* Reduce padding for specs sections */
...
function convertComparisonMarkdownToHTML(markdown, frontmatter) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inProductSection) {
            ...
            // Handle specifications table - only add if not empty
            else if (line.includes('specs-table') && line.trim() && !line.includes('<table class="specs-table"></table>')) {
                processedLines.push(`<div class="product-info-section">${line}</div>`);
            }
            // Handle key features
