...
function convertComparisonMarkdownToHTML(markdown, frontmatter) {
    ...
    for (let i = 0; i < lines.length; i++) {
            ...
            if (products.includes(productName)) {
                ...
                for (let j = i + 1; j < Math.min(i + 3, lines.length); j++) {
                    if (lines[j].includes('<img')) {
                        processedLines.push(lines[j].replace('content-image', 'product-image'));
                        i = j;
                        break;
                    }
                }
        ...
        if (inProductSection) {
            ...
            if (line.startsWith('**Price:**') || line.startsWith('**Release Date:**') || 
                ...
                if (label === 'Price') {
                    processedLines.push(`<div class="price-tag">${value}</div>`);
                } else {
                    processedLines.push(`<p><strong>${label}:</strong> ${value}</p>`);
                }
                processedLines.push('</div>');
            }
            // Handle affiliate button
            else if (line.includes('affiliate-btn')) {
                processedLines.push('<div class="affiliate-section">');
                processedLines.push(line);
                processedLines.push('</div>');
            }
            // Handle Specifications
            else if (line.includes('#### Specifications')) {
                inSpecs = true;
                inFeatures = false;
                processedLines.push('<div class="product-info-section">');
            ...
            // Handle Video Review
                ...
                inCons = false;
                processedLines.push('<div class="product-info-section">');
                processedLines.push('<h4 class="info-title">🎥 Video Review</h4>');
                processedLines.push('<div class="video-wrapper">');
            }
            // Handle specification table rows
            ...
            // Handle main comparison table
            if (line.startsWith('|') && line.includes('Feature') && lines[i + 1] && lines[i + 1].includes('---')) {
                processedLines.push('<div class="comparison-section">');
                processedLines.push('<h2 class="section-title">📋 Detailed Comparison</h2>');
                processedLines.push('<table class="comparison-table">');
                ...
                headerCells.forEach((cell, index) => {
                    if (index === 0) {
                        processedLines.push('<th class="feature-cell">Feature</th>');
                    } else {
                        processedLines.push(`<th>${cleanCellContent(cell)}</th>`);
                    }
                });
                processedLines.push('</tr>');
                ...
                for (let j = i + 1; j < lines.length; j++) {
                    ...
                    if (cells.length > 0) {
                        ...
                        cells.forEach((cell, index) => {
                            if (index === 0) {
                                processedLines.push(`<td class="feature-cell">${cleanCellContent(cell)}</td>`);
                            } else {
                                processedLines.push(`<td>${cleanCellContent(cell)}</td>`);
                            }
                        });
                        processedLines.push('</tr>');
                    }
            ...
            // Handle side-by-side comparison video
                ...
                processedLines.push('<div class="comparison-section">');
                processedLines.push('<h2 class="section-title">📺 Side-by-Side Comparison Video</h2>');
            ...
            else if (line.startsWith('## ') && !line.startsWith('###') && !line.includes('Side-by-Side')) {
                processedLines.push(`<h3>${line.replace('## ', '')}</h3>`);
            }
            // Regular content outside product sections
            else if (line.startsWith('<') || line.startsWith('- ') || line.startsWith('<li>')) {
                processedLines.push(line);
            } else if (!line.startsWith('#') && !line.startsWith('|') && !line.startsWith('<')) {
